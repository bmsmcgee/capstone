...
int16_t bq35100_read_all_blocking(int16_t *voltage, int16_t *current, float *temp, int16_t *soh)
{
    ...
    ret = bq35100_read_word(BQ35100_REG_VOLTAGE);
    if (ret < 0)
        return ret;
    *voltage = ret;

    // Get the current value
    ret = bq35100_read_word(BQ35100_REG_CURRENT);
    if (ret < 0)
        return ret;
    *current = (int16_t)ret;

    // Get the internal temperature value
    ret = bq35100_read_word(BQ35100_REG_TEMPERATURE);
    if (ret < 0)
        return ret;
    // Convert from 0.1 Kelvin to Celsius
    *temp = ((float)ret / 10.0f) - 273.15f;

    // Get the State of Health value
    ret = bq35100_read_word(BQ35100_REG_SOH);
    if (ret < 0)
        return ret;
    *soh = ret;
